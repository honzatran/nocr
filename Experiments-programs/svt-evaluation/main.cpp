...
int main( int argc, char **argv )
{
    ...
    if ( !svt.empty() )
    {
        StreetViewGroundTruth svt_records;
        if (svt.back() != '/')
        {
            svt += '/';
        }

        svt_records.loadFromFile( svt + "test.xml" );
        er_testing.loadGroundTruth(&svt_records);
        mser_testing.loadGroundTruth(&svt_records);
        
        auto samples = svt_records.getSamples();

        for ( const auto & test_sample : samples )
        {
            dictionary.clearDictionary();
            string file_path = svt + test_sample.file_name_;
            cout << file_path << endl;
            cv::Mat image = cv::imread( file_path, CV_LOAD_IMAGE_COLOR );

            string file_name = getFileName( file_path );

            for (const string & w : test_sample.dictionary)
            {
